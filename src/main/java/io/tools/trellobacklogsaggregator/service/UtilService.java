...
public class UtilService {
    ...
    private static void initDate() {
        ...
        for (int i = 1; i < 13; i++) {
            ...
            final MonthRest monthRest = new MonthRest(getDisplayMonth(tmp), tmp.getMonthValue(), tmp.lengthOfMonth(), 0, tmp.getYear());
...
//            DateTime result = dateTime.dayOfWeek().setCopy(DateTimeConstants.MONDAY);
            for (int j = 0; j <= weeks; j++) {
                if (result.getDayOfWeek() < DateTimeConstants.SATURDAY) {
                    final DateTime dayOfMonth = result.dayOfWeek().setCopy(DateTimeConstants.FRIDAY);
                    monthRest.addWeek(result.getDayOfMonth(), dayOfMonth.getDayOfMonth(), j, result.getDayOfWeek(), dayOfMonth.getDayOfWeek());
                }
    ...
    public static int getDaysOfWeek(int month, int week, int day) {
        int tmp = 0;
        ...
        if(first.isPresent()) {
            ...
            if (weekRest.isPresent()) {
                ...
                if (weekRest1.getIndexFirstDay() == day) {
                    tmp = weekRest1.getFirstDay();
                } else if (weekRest1.getIndexLastDay() == day) {
                    tmp = weekRest1.getLastDay();
                } else if (day < weekRest1.getIndexFirstDay()) {
                    tmp = 0;
                } else {
                    tmp = weekRest1.getFirstDay() + (day - weekRest1.getIndexFirstDay());
                }
            }
        }
        return tmp;
    ...
    public static int getWeekOfDay(LocalDate date){
        ...
        if(currentMonth.isPresent()){
            Optional<WeekRest> weekRestOptional = currentMonth.get().getWeekRestList().stream()
                    .filter(item ->
                            date.getDayOfMonth() >= item.getFirstDay() && date.getDayOfMonth() <= item.getLastDay()
                    )
                    .findFirst();
